...
def dashboard(user):
    ...
    with db.connect() as conn:
        ...
        if request.method == 'POST' and 'code' in request.form and 'workshop_num' in request.form:
            ...
            update_statement = "UPDATE attendance SET Workshop_5 = :update_num WHERE email_address = :email_address;"
            ...
            with db.connect() as conn:
                update_statement = "UPDATE assignments SET Workshop_4 = :worksheet_link WHERE email_address = :email_address;"
                values = {'worksheet_link': worksheet_link,'email_address': user}
                conn.execute(text(update_statement), values)

                user_assignments[3] = worksheet_link
